...
Ветка: **`feature/midi-sequencer`** (запушена в origin), базовый коммит `90eb4da2c`.
Коммит `d077037ac` — этапы 0–3. Правки stage-доков после него коммитить туда же.
⚠️ Ловушка: пункты меню из динамических action-строк — `makeMenuItem` возвращает **nullptr**
для незарегистрированного экшена (SIGSEGV); либо UiAction на полную строку (как цвета клипов),
либо обычный код + `item->setArgs(ActionData)` (как midi-set-instrument/midi-render).
